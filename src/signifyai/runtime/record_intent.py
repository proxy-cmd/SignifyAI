...
class IntentRecorder:
    def __init__(self, cfg: RecordConfig) -> None:
        ...
        self.store = RecStore(RecStoreCfg(root=Path("data/landmarks")))
    ...
    def run(self) -> dict:
        ...
        try:
            while clips_saved < self.cfg.clips:
                ...
                line1 = f"Intent: {self.cfg.intent_id}"
                line2 = f"Clips: {clips_saved}/{self.cfg.clips} | SPACE capture | q quit"
                cv2.putText(frame, line1, (18, 34), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (240, 240, 240), 2)
                cv2.putText(frame, line2, (18, 66), cv2.FONT_HERSHEY_SIMPLEX, 0.54, (210, 210, 210), 2)
                ...
                if key == 32:
                    ...
                    while (time.time() - start) < self.cfg.clip_seconds:
                        ...
                        cv2.waitKey(1)
                    result = self.store.append_frames(frames)
        ...
        final = self.store.finalize_clip()
