...
class Rules20:
    """Hardcoded sign recognizer for a small demo vocabulary."""
    ...
    def _single_hand(self, hand: np.ndarray) -> Optional[Prediction]:
        ...
        if open_palm:
            if self._is_wave(hand, open_palm):
                return Prediction("HELLO", 0.95)
            return Prediction("HELLO", 0.90)
        ...
        if others_folded:
            ...
            mostly_vertical = abs(dx) <= (abs(dy) * 1.2 + 0.01)
            if mostly_vertical and thumb_tip[1] < wrist[1] - 0.06:
                return Prediction("YES", 0.90)
            if mostly_vertical and thumb_tip[1] > wrist[1] + 0.06:
                return Prediction("NO", 0.90)

        if s["index"] and not s["middle"] and not s["ring"] and not s["pinky"] and not s["thumb"]:
        ...
        if ok_dist < 0.045 and s["middle"] and s["ring"] and s["pinky"]:
            return Prediction("OKAY", 0.89)
        ...
        if s["index"] and s["pinky"] and not s["middle"] and not s["ring"]:
            thumb_far = self._dist(hand[TIP["thumb"]], hand[MCP["index"]]) > 0.12
            if s["thumb"] and thumb_far:
                return Prediction("I LOVE YOU", 0.88)
    ...
    def predict(self, hand_count: int, raw_hands: list[np.ndarray]) -> Optional[Prediction]:
        ...
        if hand_count >= 2 and len(raw_hands) >= 2:
            ...
            if self._is_open_palm(s0) and self._is_open_palm(s1):
                ...
                avg_y = float((c0[1] + c1[1]) * 0.5)
                if dist < 0.20:
                    return Prediction("THANK YOU", 0.88)
                if dist > 0.27 and avg_y < 0.42:
                    return Prediction(self._time_greeting(), 0.86)
                return Prediction("THANK YOU", 0.84)
